...
export function PageHeader({ title, breadcrumbs, actions }: { title: string; breadcrumbs?: { label: string; href?: string }[]; actions?: ReactNode; }) {
  return (
          ...
          <nav className="mt-1 flex items-center gap-2 text-sm text-muted-foreground">
            {breadcrumbs.map((b, i) => (
              <span key={i} className="flex items-center gap-2">
                {i > 0 && <span className="text-border">/</span>}<span>{b.label}</span>
              </span>
            ))}
          </nav>
        )}
